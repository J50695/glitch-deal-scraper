...
async function sendDiscordAlert(deal) {
  ...
  const embed = {
    ...
    url:         deal.url || '',
    color:       color,
    description: `**${deal.retailer}** price glitch detected!`,
    fields: [
      { name: 'Glitch Price', value: fmtPrice(deal.price),         inline: true },
      { name: 'Normal Price', value: fmtPrice(deal.originalPrice), inline: true },
      { name: 'You Save',     value: fmtPrice(deal.savings),       inline: true },
    ],
...
async function sendEmailDigest(deals) {
  ...
  const dealCards = deals.map(d => `
    <div style="border:2px solid #FF3333;border-radius:12px;padding:16px;margin:12px 0;background:#1a1a1a">
      ...
      <p>Glitch: <b style="color:#FF3333">${fmtPrice(d.price)}</b> | Normal: <span style="text-decoration:line-through;color:#999">${fmtPrice(d.originalPrice)}</span></p>
